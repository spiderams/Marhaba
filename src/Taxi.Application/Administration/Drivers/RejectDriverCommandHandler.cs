...
internal sealed class RejectDriverCommandHandler(IRepository<Driver> drivers)
    : ICommandHandler<RejectDriverCommand, DriverDto>
{
    public async Task<Result<DriverDto>> Handle(RejectDriverCommand command, CancellationToken cancellationToken)
    {
